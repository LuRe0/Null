...
//------------------------------------------------------------------------------
//
// File Name:	CubeInstanceMesh.cpp
...
#define GL(function) \
	...
	CubeInstanceMesh::CubeInstanceMesh(const std::string& filename,const uint32_t vertexCount, const uint32_t indexCount) : Mesh()
	{
		Read(filename);
		...
		uint32_t offset = 0;
		for (size_t i = 0; i < indexData.size(); i += 36)
		{
			...
			indexData[i + 6] = offset + 4;
			indexData[i + 7] = offset + 5;
			indexData[i + 8] = offset + 6;

			indexData[i + 9] = offset + 6;
			indexData[i + 10] = offset + 7;
			indexData[i + 11] = offset + 4;

			indexData[i + 12] = offset + 8;
			indexData[i + 13] = offset + 9;
			indexData[i + 14] = offset + 10;

			indexData[i + 15] = offset + 10;
			indexData[i + 16] = offset + 11;
			indexData[i + 17] = offset + 8;

			indexData[i + 18] = offset + 14;
			indexData[i + 19] = offset + 13;
			indexData[i + 20] = offset + 12;

			indexData[i + 21] = offset + 12;
			indexData[i + 22] = offset + 15;
			indexData[i + 23] = offset + 14;

			indexData[i + 24] = offset + 18;
			indexData[i + 25] = offset + 17;
			indexData[i + 26] = offset + 16;

			indexData[i + 27] = offset + 16;
			indexData[i + 28] = offset + 19;
			indexData[i + 29] = offset + 18;

			indexData[i + 30] = offset + 20;
			indexData[i + 31] = offset + 21;
			indexData[i + 32] = offset + 22;

			indexData[i + 33] = offset + 22;
			indexData[i + 34] = offset + 23;
			indexData[i + 35] = offset + 20;
	...
	void CubeInstanceMesh::Bind() const
	{
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_VBO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void CubeInstanceMesh::Unbind() const
	{
	...
	void CubeInstanceMesh::SetupIndexBuffer(const std::vector<unsigned int>& indexData) {
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void CubeInstanceMesh::SetupVertexAttributes() 
	{
	...
	void CubeInstanceMesh::SetupInstances()
	{
	...
	void CubeInstanceMesh::Render(uint32_t count) const
	{
		m_Buffer.m_VAO.Bind();
