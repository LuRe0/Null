...
namespace NULLENGINE
{
	...
	public:
		TriangleBatchRenderer(std::size_t count)
		{
			...
			m_InstanceMesh = std::make_unique<TMesh>("Triangle", m_MaxVertices, m_MaxIndices);
			m_InstanceBuffer.reserve(m_MaxVertices);
		}

		~TriangleBatchRenderer() override
		{
		}
	...
	template <typename TInstance, typename TMesh>
	inline void TriangleBatchRenderer<TInstance, TMesh>::NextBatch(Shader* shader)
	{
		Flush(shader);
		BeginBatch();
	}
	...
	template <typename TInstance, typename TMesh>
	inline void TriangleBatchRenderer<TInstance, TMesh>::Flush(Shader* shader)
	{
		...
		//std::string shaderName = "triInstance";
	...
	template <typename TInstance, typename TMesh>
	inline void TriangleBatchRenderer<TInstance, TMesh>::AddInstance(const ElementData& render, Shader* shader)
	{
	...
	inline void TriangleBatchRenderer<TInstance, TMesh>::ImguiView()
	{
		...
		ImGui::Text("Render Type: %s", "TRIANGLES");
		ImGui::Text("Draw Calls: %d", m_Stats.DrawCalls);
		ImGui::Text("Quads: %d", m_Stats.InstanceCount);
		ImGui::Text("Vertices Calls: %d", m_Stats.InstanceCount * 3);
		ImGui::Text("Indices Calls: %d", m_Stats.InstanceCount * 3);
	...
	template <typename TInstance, typename TMesh>
	inline void TriangleBatchRenderer<TInstance, TMesh>::BindTextureBuffer(Shader* shader)
	{
