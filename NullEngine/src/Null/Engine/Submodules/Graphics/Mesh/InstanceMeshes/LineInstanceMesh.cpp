...
//------------------------------------------------------------------------------
//
// File Name:	LineInstanceMesh.cpp
...
#define GL(function) \
	...
	LineInstanceMesh::LineInstanceMesh(const std::string& filename,const uint32_t vertexCount, const uint32_t indexCount) : Mesh()
	{
		Read(filename);
	...
	void LineInstanceMesh::Bind() const
	{
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_VBO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void LineInstanceMesh::Unbind() const
	{
	...
	void LineInstanceMesh::SetupIndexBuffer(const std::vector<unsigned int>& indexData) {
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void LineInstanceMesh::SetupVertexAttributes() 
	{
	...
	void LineInstanceMesh::SetupInstances()
	{
