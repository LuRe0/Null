...
//------------------------------------------------------------------------------
//
// File Name:	TriangleInstanceMesh.cpp
...
//******************************************************************************//
#include "stdafx.h"
#include "TriangleInstanceMesh.h"
#include "glad/glad.h"
#include "Null/Core.h"
...
#define GL(function) \
	...
	TriangleInstanceMesh::TriangleInstanceMesh(const std::string& filename,const uint32_t vertexCount, const uint32_t indexCount) : Mesh()
	{
		Read(filename);
	...
	TriangleInstanceMesh::~TriangleInstanceMesh()
	{

	}

	void TriangleInstanceMesh::Bind() const
	{
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_VBO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void TriangleInstanceMesh::Unbind() const
	{
	...
	void TriangleInstanceMesh::SetupIndexBuffer(const std::vector<unsigned int>& indexData) {
		m_Buffer.m_VAO.Bind();
		m_Buffer.m_EBO.Bind();
	...
	void TriangleInstanceMesh::SetupVertexAttributes() 
	{
	...
	void TriangleInstanceMesh::SetupInstances()
	{
	...
	void TriangleInstanceMesh::Render(uint32_t count) const
	{
		m_Buffer.m_VAO.Bind();
